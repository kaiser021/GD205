...
public class grid_movement_1 : MonoBehaviour
{
    ...
    { //use this function to check the players position for hazards
        for (int i = 0; i < hazards.Length; i++)
        { //use the loop to check for each hazard in the array
            if (playerTransform.position == hazards[i].position)
            { //if the player position is the same as this current hazard in our loop
    ...
    void createTrail()
    { //check for if the player is in each field child to create the trail
        for (int i = 0; i < fieldParent.childCount; i++)
        { //loop through each child
            if (playerTransform.position == fieldParent.GetChild(i).position)
            { //if the players position is the same as the current
